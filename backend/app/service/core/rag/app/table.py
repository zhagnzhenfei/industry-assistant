...
# from api.db.services.knowledgebase_service import KnowledgebaseService  # 注释掉不存在的导入
from service.core.deepdoc.parser.utils import get_text
from service.core.rag.nlp import rag_tokenizer, tokenize
from service.core.deepdoc.parser import ExcelParser
...
def chunk(filename, binary=None, from_page=0, to_page=10000000000, lang="Chinese", callback=None, **kwargs):
    ...
    for df in dfs:
        ...
        # KnowledgebaseService.update_parser_config(kwargs["kb_id"], {"field_map": {k: v for k, v in clmns_map}})  # 注释掉不存在的服务调用
    callback(0.35, "")
